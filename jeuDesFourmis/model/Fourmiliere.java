...
public class Fourmiliere {
  ...
  public Fourmiliere(int l, int h, int qMax) {
    ...
    for (int i =0 ; i < hauteur+2 ; i++)
      for (int j =0 ; j < largeur+2 ; j++)
	murs[i][j] = (i==0)||(j==0)|| j+1 == largeur || i+1 ==hauteur ;
  ...
  //modifie les valeurs de la partie actuel par une autre partie ( pas utilisable à cette version )
    public void setGame(Fourmiliere fourmiliere){
        int oldWall = fourmiliere.hauteur;
        for (int i =0 ; i < hauteur+2 ; i++){
            for (int j =0 ; j < largeur+2; j++){
               
                if(i<fourmiliere.hauteur-1 && j<fourmiliere.largeur-1 && i>0 && j>0){
                    fourmis[i][j] = fourmiliere.contientFourmi(i, j);
                    qteGraines[i][j] = fourmiliere.getQteGraines(i, j);
                    murs[i][j] =fourmiliere.getMur(i, j);
                }
            }
        }
        ...
        for (int i =0 ; i < hauteur+2 ; i++)
            for (int j =0 ; j < largeur+2 ; j++)
                murs[i][j] = (i==0)||(j==0)|| j+1 == largeur || i+1 ==hauteur ;
        this.lesFourmis = fourmiliere.getFourmis();
  ...
  public Fourmi getFourmis(int x ,int y ){
      for(Fourmi f : this.lesFourmis){
  ...
  public int getGraine(int x , int y ){
       return qteGraines[x][y] ;
  }
  /**
